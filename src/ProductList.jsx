...
function ProductList({ onHomeClick }) {
    ...
    console.log("prodct list",cart)
    const dispatch = useDispatch();
    const [showCart, setShowCart] = useState(false);
    const [showPlants, setShowPlants] = useState(false); // State to control 
    
    //the visibility of the About Us page
    const  [productQuantity,setProductQuantity] = useState(0);
    function calculateQantity(cart){
        setProductQuantity( cart.reduce((acc,obj)=>(acc + obj.quantity),0))   
    }
  
    useEffect(()=>{
      calculateQantity(cart);
    },[cart])
    const plantsArray = [
        {
    ...
    return (
        <div className='productList-container'>
            ...
            {!showCart ? (
                plantsArray.map((productList, index) => (
                    <div key={productList.index}>
                        <h2 className='product-category'><span>{productList.category}</span></h2>
                        <div className="product-grid">
                            <div className='product-list'>
                                {productList.plants.map((product) => (
